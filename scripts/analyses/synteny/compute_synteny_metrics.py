...
def parse_pair_collinearity(colinearity_file, sp1, sp2, homoeolog_pairs=None):
    ...
    Skip blocks between homoeolog pairs (if not None).
    """
...
def compute_synteny_metrics(gff_file, collinearity_file, sp1, sp2, total_genes,
    ...
    - Breakpoints: number of gaps between distinct colinear blocks along a
        chromosome (excluding extremities).
    ...
    n_breakpoints = 0
    for sp in (sp1, sp2):
        for chrom, n_blocks_chrom in blocks_per_chrom[sp].items():
            n_breakpoints += max(n_blocks_chrom - 1, 0)
...
def export_all_matrices(results_tsv, species, out_dir, out_prefix):
    """
    Export all matrices to distinct tsv files.
    """
